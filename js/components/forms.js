// ─────────────────────────────────────────────────────────────────────────────
// forms.js — modal form components for creating/editing tasks, events, projects
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        {/* Urgency slider — colour matches the urgency level */}
...
// ── EventForm ─────────────────────────────────────────────────────────────────
// Fixed appointments (e.g. meetings) — have a specific date + start time,
// unlike tasks which the scheduler places automatically.
...
// ── ProjectForm ───────────────────────────────────────────────────────────────
// Creates or edits a project container (name, optional due date, colour swatch).
function ProjectForm({init={},onSave,onClose}){
  const[f,setF]=useState({name:"",dueDate:"",color:C.red,...init});
  const s=(k,v)=>setF(p=>({...p,[k]:v}));
  const COLS=[C.red,C.blue,C.orange,C.green,C.yellow]; // available project colours
  return(
    <div>
      ...
      <Lbl t="COLOR">
        <div style={{display:"flex",gap:8,marginTop:4}}>
          {/* Colour swatches — white border indicates the selected colour */}
