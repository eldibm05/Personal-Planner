// ─────────────────────────────────────────────────────────────────────────────
// LoginScreen.js — password lock screen shown before the main app
// On success, writes an auth flag to localStorage so the user stays logged in
// across page refreshes. The 🔒 button in App clears that flag.
// ─────────────────────────────────────────────────────────────────────────────
function LoginScreen({onUnlock}){
  ...
  const attempt=()=>{
    if(pw===PASSWORD){
      localStorage.setItem(AUTH_KEY,"1"); // persist the logged-in state
      onUnlock();
    } else {
      setErr(true);
      setShake(true);
      setPw("");
      playBeep("warn");
      setTimeout(()=>setShake(false),500); // reset shake after animation completes
    }
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",background:C.bg,gap:24}}>
      ...
      {/* Password box — shakes on wrong attempt */}
      <div style={{border:`3px solid ${C.blue}`,background:C.bg2,padding:"28px 32px",maxWidth:320,width:"90%",animation:shake?"shake .4s ease":"none"}}>
        ...
        <Inp
          ...
          onKeyDown={e=>e.key==="Enter"&&attempt()} // submit on Enter
      ...
      {/* Shake keyframe — defined inline so it travels with this component */}
